...
- The same GenericGymEnv / rtgym real-time stepping pipeline as the DDPG RolloutWorker
  (same time_step_duration, ep_max_length, act_buf_len, act_in_obs as ddpg/config.py).
...
NEVER loads or references the pretrained SAC model or the DDPG actor. NEVER feeds data into
DDPG_MEMORY / dataset_ddpg. NEVER sends synthetic control (see TM2020InterfaceLidarHuman).
"""
...
from ddpg.config import DDPG_OBS_PREPROCESSOR
from ddpg.human_interface import HUMAN_ENV_CLS, HUMAN_DATASET_PATH
from ddpg.human_input import KeyboardHumanController
...
def _reset_sample(env, buffer):
    ...
    new_obs = DDPG_OBS_PREPROCESSOR(new_obs)

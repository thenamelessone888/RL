...
Algorithm-agnostic: recorded human driving data is independent of which RL algorithm will
later consume it (behavior cloning, reward shaping, etc.), so this module builds its own
RTGym config directly from TMRL's defaults rather than importing any specific algorithm's
config module (e.g. ddpg/config.py or td3/config.py) -- avoiding a hard dependency of the
recording pipeline on any one algorithm package being present/importable.
...
# Reuse the same img_hist_len/gamepad settings any LIDAR algorithm config in this project uses.
HUMAN_INT = partial(TM2020InterfaceLidarHuman, img_hist_len=cfg.IMG_HIST_LEN, gamepad=cfg.PRAGMA_GAMEPAD)

# Built directly from TMRL's own RTGYM defaults + the shared config.json RTGYM_CONFIG section
# (time_step_duration, ep_max_length, act_buf_len, act_in_obs, ...) -- the exact same
# construction every algorithm's own *_CONFIG_DICT uses (see e.g. td3/config.py), duplicated
# here rather than imported so recording never depends on any one algorithm package being
# importable. Only the interface class differs.
HUMAN_CONFIG_DICT = rtgym.DEFAULT_CONFIG_DICT.copy()
HUMAN_CONFIG_DICT["interface"] = HUMAN_INT
for key, value in cfg.ENV_CONFIG["RTGYM_CONFIG"].items():
    HUMAN_CONFIG_DICT[key] = value

HUMAN_ENV_CLS = partial(GenericGymEnv, id=cfg.RTGYM_VERSION, gym_kwargs={"config": HUMAN_CONFIG_DICT})

# ISOLATED dataset directory: never the SAC dataset, never any algorithm's replay dataset.
...
# Per-algorithm dataset isolation (e.g. TD3_DATASET_PATH != cfg.DATASET_PATH) is asserted in
# each algorithm's own config module (see td3/config.py); this module only owns the human
# dataset path and does not need to know which algorithm packages happen to be installed.